...
//#define IDX(i, j) (WRAPPED(i, MX) + WRAPPED(j, MY) * MX)
#define IDX(i, j) spatial_to_morton((uint2_64){WRAPPED(i, MX), WRAPPED(j, MY)})

#define IS_POWER_OF_TWO(val) ((val & (val - 1)) == 0)
// static_assert(IS_POWER_OF_TWO(MX));
// static_assert(IS_POWER_OF_TWO(MY));
...
#pragma omp parallel for
        for (int i = NX_MIN; i < NX_MAX; i++) {
            for (int j = NY_MIN; j < NY_MAX; j++) {
                curr_p[IDX(i, j)] = beta * (alpha * div[IDX(i, j)] +
                                            prev_p[IDX(i - 1, j)] +
                                            prev_p[IDX(i + 1, j)] +
                                            prev_p[IDX(i, j - 1)] +
                                            prev_p[IDX(i, j + 1)]);
    ...
    for (int k = 0; k < 20; k++) {
        for (int i = NX_MIN; i < NX_MAX; i++) {
            for (int j = NY_MIN; j < NY_MAX; j++) {
                p[IDX(i, j)] = .25 * (div[IDX(i, j)] + p[IDX(i - 1, j)] +
                                      p[IDX(i + 1, j)] + p[IDX(i, j - 1)] +
                                      p[IDX(i, j + 1)]);
            }
        }
        // set bounds for p
...
#pragma omp parallel for
    for (size_t i = 0; i < MXY; ++i) {
        rho[i] = input.field[LNRHO].data[i];
    ...
    #define IX(i, j) spatial_to_morton((uint2_64){i, j})
        ...
        for (size_t j = 0; j < NY; ++j)
            for (size_t i = 0; i < NX; ++i) {
                ...
                output->field[LNRHO].data[dst] = rho[src];
...
#pragma omp parallel for
    for (size_t i = 0; i < MXY; ++i) {
        output->field[LNRHO].data[i] = rho[i];
